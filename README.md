...
- [오프라인 데이터 모델](docs/OFFLINE_DATA_MODEL.md): 문제와 세션 분리, v4 저장 형식, 기존 데이터와 클라우드 백업 이전
...
- OX, 5지선다, 단답형 CSV 업로드와 여러 파일 일괄 등록
- 오프라인 과목, 문제, 세션의 3단계 구성과 문제 원본을 공유하는 독립 풀이 기록
- 문제 등록 시각, 세션 생성과 마지막 풀이 시각 표시
...
- 오답 노트 작성과 저장, 온라인 저장 실패 시 현재 문항과 작성 중인 노트 보존
...
- 다크 모드와 글꼴 설정 (Pretendard, Noto Sans KR, 나눔고딕, 나눔명조)
...
│   │   ├── study/                      # 풀이와 복습의 보기, 번호, OMR 공통 표시 UI
...
- 집중 풀이 화면: 단색 레드 CTA와 90ms의 짧은 hover 색상 전환 사용. 문항 이동, 내용에 따른 박스 높이와 모바일 OMR에만 짧은 전환 효과 적용
- 문제 카드와 세션 목록: 문제는 흰색 카드, 풀이 기록은 우측 CTA가 있는 공통 목록으로 표시
...
풀이와 복습은 별도의 `app-study-page` 범위에서 16px 패널, 12px 선지와 내부 박스, 8px 태그와 OMR 내부 표를 공유합니다. 5지선다 번호는 `StudyChoiceNumber`의 16px 원 안에 10px 숫자로 표시합니다. 보기, 선지와 OMR은 공통 표시 컴포넌트를 사용하고 답안 저장, 화면 이동과 오답 노트 관리는 각 화면이 계속 담당합니다. `/debug/designsystem#ds-study`에서 실제 컴포넌트의 선택, 채점과 복습 상태를 확인할 수 있으며 예시 상태는 저장되지 않습니다. 자세한 범위와 예외는 [디자인 시스템](docs/DESIGN_SYSTEM.md)을 따릅니다.
...
- `/dashboard/:subjectId`: 과목별 문제 카드, CSV 등록, 문제 제목과 과목 편집
- `/dashboard/:subjectId/problem-sets/:problemSetId`: 문제별 풀이 세션 목록과 새 세션 생성
...
문제 원본, 세션별 풀이 기록, 과목은 IndexedDB의 `law-solver-offline` 데이터베이스, `persisted-state` object store에 `law-solver-storage` 키로 저장됩니다. 환경설정은 기존처럼 `law-solver-settings` 키로 localStorage에 저장됩니다. 화면 설정에서 오프라인 문제 목록의 제목/등록 기준과 오름차순/내림차순을 선택할 수 있으며, 기본은 등록 내림차순입니다. 변경한 선택은 브라우저에 자동 저장됩니다. 같은 화면의 자동 하이퍼링크에서 해설과 출처의 판례번호 링크를 끄거나 국가법령정보센터와 케이스노트를 선택할 수 있습니다. 기본은 국가법령정보센터이며 판례번호를 누르면 새 탭으로 열립니다. 오프라인과 온라인의 풀이, 전체 복기와 오답 확인에서 해설과 출처에 함께 적용됩니다.
...
`과목 없음`은 실제 과목 객체로 저장하지 않습니다. 문제의 `subject_id`가 `null`인 상태입니다. CSV 등록은 문제만 만들며, 해당 문제 안에서 새 풀이 세션을 생성합니다. 문항 본문은 문제 원본에 한 번 보관하고 세션마다 문항 순서, 답안, 오답 노트, 책갈피와 결과를 별도로 저장합니다.

저장과 백업 형식은 v4입니다. 기존 세션은 각각 문제 하나와 세션 하나로 이전하며 같은 제목도 합치지 않습니다. 문제 등록 시각과 이후 세션의 마지막 풀이 시각을 기록하고, 기존에 없던 마지막 풀이 시각은 추정하지 않습니다. 자세한 모델과 이전 절차는 [OFFLINE_DATA_MODEL.md](docs/OFFLINE_DATA_MODEL.md)를 참고하세요.
...
| `problem_upload_completed` | CSV 문제 등록 성공. 일괄 등록은 파일마다 전송 | `question_type` |
...
- 과목 목록, 표지 색상과 표시 순서
- 문제 원본과 소속 과목, 등록 시각
- 세션별 문항 순서, 답안, 풀이 시간과 결과
- 세션별 오답 노트, 책갈피와 마지막 풀이 시각
...
이전 배열 백업과 v1부터 v3까지의 백업도 복원할 수 있습니다. 기존 세션마다 문제 하나와 세션 하나로 변환하고 유효한 과목 연결을 보존합니다. 과목 정보가 없는 배열 백업은 `과목 없음`으로 들어갑니다. 클라우드에 보관된 구형 백업도 복호화 후 같은 검사와 이전을 거치며, 최종 확인과 영구 저장 성공 전에는 현재 데이터를 바꾸지 않습니다.
...
새 문제 등록에서 CSV 파일을 선택하면 확장자를 제외한 파일명이 문제 제목으로 입력됩니다. 등록한 문제에서 세션을 만들 때 풀이 순서와 세션 제목을 정합니다. 파일명의 특수문자는 공백으로 바뀌며 연속 공백은 하나로 정리됩니다. 선택지 헤더가 있으면 5지선다로, 선택지 헤더가 없고 정답이 모두 O/X 계열이면 OX로, 그 밖의 정답 문자열은 단답형으로 자동 선택합니다. 형식을 판별할 수 없으면 현재 선택한 문제 타입을 유지합니다.

여러 CSV 파일을 선택하면 파일 수를 확인한 뒤 일괄 등록할 수 있습니다. 각 파일의 이름과 내용으로 제목과 유형을 자동 지정하며 현재 과목에 파일마다 별도 문제를 만듭니다. 제목으로 쓸 문자가 없는 파일은 선택 순서대로 `새 문제 1`처럼 이름을 붙입니다. 모든 파일을 확인한 뒤 함께 등록하므로 오류가 있으면 일부만 등록되지 않습니다. 유형을 자동으로 알 수 없는 파일은 하나만 선택해 유형을 지정해 주세요. 등록 후에는 현재 문제 목록에 머물며, 풀이 세션은 문제별로 직접 만들 수 있습니다.
...
- `정답`: `1`부터 `5`
- 복수정답은 `1,2` 또는 `1, 2`처럼 입력합니다. 이 중 하나를 고르면 정답입니다. 쉼표 주변 공백과 중복 번호는 정리하며, `2,1,2`는 `1,2`로 저장합니다.
- `정답`이 `0`이면 정답 없음입니다. 어떤 답을 골라도, 답을 고르지 않아도 정답으로 처리합니다. `0,1`처럼 다른 번호와 함께 쓰는 형식은 허용하지 않습니다.
- CSV 원문에서 쉼표를 포함한 셀은 `"1,2"`처럼 큰따옴표로 감싸야 합니다. 엑셀에서는 셀에 `1,2`를 입력하고 CSV로 저장하면 됩니다.
- `해설`과 `출처` 열은 생략할 수 있습니다. 이름으로 헤더를 식별한 CSV에서는 빠진 항목을 빈 값으로 처리합니다.
- `선택지` 대신 `지문`, `선지`, `choice`, `option` 계열 헤더도 일부 호환됩니다.
- 박스형 지문은 `박스1`, `박스2` 또는 `box1`, `box2` 같은 헤더로 추가할 수 있습니다.

복수정답과 정답 없음은 채점, 결과 통계, 풀이 중 정답 보기, 오답 확인, 전체 확인, 오답 재풀이와 오답 CSV에 같은 기준으로 반영됩니다. 정답 없음 문항은 결과의 정답 수에 포함하고 오답이나 미응답 수에는 중복 집계하지 않습니다. 단답형은 쉼표나 `0`도 답안의 일부로 비교하며 기존 문자열 일치 규칙을 유지합니다.
...
- 과목 삭제 시 문제와 세션은 삭제되지 않고 `과목 없음`으로 이동합니다. 문제 삭제는 소속 세션도 함께 삭제하며, 개별 세션 삭제는 문제 원본을 유지합니다.
- v4 저장 형식을 구형 앱으로 되돌리는 하향 이전은 지원하지 않습니다. 롤백도 v4를 이해하는 버전으로 진행해야 합니다.
