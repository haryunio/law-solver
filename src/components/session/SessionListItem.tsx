...
export function SessionListItem({
  ...
  return (
    <article className="app-card app-problem-card relative rounded-xl border px-3 py-4 sm:px-4 lg:flex lg:items-end lg:gap-2">
      <div className="min-w-0 flex-1 lg:flex lg:items-end lg:gap-4">
        <div className="grid min-h-14 min-w-0 flex-1 grid-cols-[56px_minmax(0,1fr)] items-center gap-x-3 gap-y-2 lg:grid-rows-[1fr_auto]">
          <span className="inline-flex h-14 w-14 shrink-0 items-center justify-center rounded-lg border border-red-100 bg-red-50 text-sm font-semibold tabular-nums text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-400 lg:row-span-2 lg:self-end">
            {attemptNumber}회차
          </span>
          ...
          <div className="col-span-2 flex min-w-0 flex-wrap items-center gap-1 lg:col-span-1 lg:col-start-2 lg:self-end">
        ...
        <div className="mt-4 flex items-center gap-2 lg:mt-0 lg:shrink-0">
          <dl className="grid min-w-0 flex-1 grid-cols-3 gap-1.5 lg:w-[270px]">
            {[
              ...
              <div key={label} className="app-neutral-box flex h-14 min-w-0 flex-col justify-center rounded-lg border px-1 sm:px-3">
          ...
          <Link
            ...
            className={`${completed ? "app-button-secondary" : "app-button-primary"} inline-flex h-14 w-24 shrink-0 items-center justify-center rounded-lg text-sm font-semibold sm:w-28`}
          >
            {completed ? "결과 확인" : "이어서 풀기"}
          </Link>
        </div>
      </div>
      {actions ? <div className="absolute right-3 top-7 sm:right-4 lg:static lg:shrink-0 lg:pb-3.5">{actions}</div> : null}
    </article>
  );
}
