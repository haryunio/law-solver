    ...
    articles: [
      ...
      },
      {
        ...
        items: [
          ...
          { text: "“오프라인 기능”이란 이용자가 자신의 CSV 파일을 업로드하여 브라우저 로컬 저장소(기본적으로 IndexedDB) 안에서 문제를 풀고 학습 기록을 관리하는 무료 기능을 말합니다." },
          { text: "“Premium 기능”이란 운영자가 서버에 등록한 문제 세트를 온라인으로 풀고 학습 기록을 서버에 저장하거나, 오프라인 데이터를 암호화하여 클라우드에 백업·복구하는 회원 기능을 말합니다." },
          ...
          { text: "“오프라인 데이터”란 이용자가 업로드한 CSV 파일, 풀이 기록, 점수, 책갈피, 오답 노트 등 브라우저 로컬 저장소에 저장되는 학습 데이터를 말합니다." },
          { text: "“클라우드 백업”이란 Premium 회원이 직접 요청하여 오프라인 데이터 전체를 브라우저에서 압축·암호화한 뒤 서버에 최신본 하나로 보관하는 기능을 말합니다." },
        ...
        clauses: [
          {
            ...
            items: [
              ...
              { text: "Premium 클라우드 백업 기능: 오프라인 데이터의 암호화된 최신본 하나를 서버에 보관하고 다른 기기에서 복구하는 회원 기능입니다." },
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        clauses: [
          { text: "오프라인 데이터는 기본적으로 이용자 기기의 브라우저 IndexedDB에 저장되며 자동으로 운영자 서버에 전송·동기화되지 않습니다. 기존 localStorage 데이터는 IndexedDB 저장과 검증이 성공한 뒤 안전하게 이전되며, IndexedDB를 지원하지 않지만 기존 localStorage 데이터가 있는 환경에서는 원본 보호를 위해 기존 저장 방식을 유지할 수 있습니다. Premium 회원이 클라우드 백업을 직접 실행한 경우에만 암호화된 사본이 전송됩니다." },
          {
          ...
          { text: "이용자는 JSON 백업 또는 Premium 클라우드 백업 기능을 이용해 중요한 데이터를 정기적으로 백업해야 합니다. 클라우드 백업을 사용하지 않은 원본은 운영자가 보유하지 않으므로 유실된 오프라인 데이터를 복구할 수 없습니다." },
          { text: "클라우드 백업은 이용자가 명시적으로 실행하며 계정별 최신본 하나만 보관합니다. 압축·암호화된 파일은 최대 15MB이고 백업과 복구는 각각 하루 5회까지 이용할 수 있습니다." },
          { text: "클라우드 백업 비밀번호는 8자 이상이어야 하며 브라우저에서 gzip 압축과 AES-256-GCM 암호화에 사용됩니다. 비밀번호·암호화 키·평문은 서버로 전송되지 않으므로 운영자는 내용을 열람하거나 분실한 비밀번호를 복구할 수 없습니다." },
          { text: "복구는 암호문 다운로드, 브라우저 복호화와 구조 검증, 현재 데이터와의 비교, 이용자의 최종 확인이 모두 끝난 경우에만 현재 브라우저 데이터를 교체합니다. 틀린 비밀번호 재시도를 위한 암호문은 현재 화면 메모리에만 임시 보관합니다." },
        ...
        clauses: [
          { text: "Premium 데이터는 Supabase 기반 서버에 저장되며 운영자는 합리적인 기술적·관리적 보호조치를 적용합니다." },
          {
            ...
            items: [
              ...
              { text: "암호화된 클라우드 백업: 이용자가 삭제하거나 탈퇴할 때까지. Premium 만료 후에는 전송 기능을 중지하고 마지막 유효 종료일로부터 1년 뒤 자동 삭제" },
    ...
    articles: [
      ...
      },
      {
        ...
        clauses: [
          ...
          { text: "질문별 답변과 점수는 서버 또는 브라우저 저장소에 저장하지 않으며 공유 URL에는 최종 4자 유형 코드만 포함합니다." },
        ],
      },
      {
        number: 23,
    ...
    articles: [
      {
        ...
        clauses: [
          ...
          { text: "개인정보에 관한 내용이 이 약관과 개인정보처리방침에서 다르면 정보주체에게 유리한 내용과 관계 법령을 우선 적용합니다." },
          {
            ...
            items: [
              { text: "오프라인 기능: 학습 데이터는 브라우저 로컬 저장소(기본적으로 IndexedDB)에 저장되며 자동 전송되지 않습니다. 이용자가 Premium 클라우드 백업을 실행하면 암호화된 사본만 서버에 저장됩니다." },
    ...
    articles: [
      {
        number: 28,
        title: "시행일",
        opening: "이 약관은 2026년 8월 1일부터 시행합니다.",
      },
      {
        number: 29,
        title: "이전 약관의 확인",
...
function LegalArticleContent({ article }: { article: LegalArticle }) {
  return (
    <section>
      ...
      {article.notice ? (
        <p className="app-radius-inset mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 font-semibold text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
          {article.notice}
        </p>
      ) : null}
