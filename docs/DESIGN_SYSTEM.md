...
풀이와 복기 화면은 장식보다 문제의 가독성을 우선합니다. 헤더 높이, 카드와 OMR의 독립 높이, 하단 이동은 기존 `CbtSolveScreen`과 복기 페이지를 기준으로 유지합니다. OMR 문항 수가 적다고 문제 카드와 같은 높이로 강제 확장하지 않습니다. 모바일 풀이의 OMR 빠른 이동은 문제 박스 우상단 `?` 왼쪽의 32px 원형 버튼으로 열며, 내부에 작은 `OMR` 글자를 표시합니다. 하단에는 이전과 다음 문제만 같은 너비로 배치합니다. 768px 이상에서는 기존 우측 OMR 패널을 사용합니다. 활성 풀이에서는 그라디언트, smooth scroll, 위치/크기 transition과 animation을 추가하지 않습니다. 상태 색상은 90ms 전환만 허용합니다.
