...
`useQuestionPanelHeightMotion`은 768px 이상에서 문항 변경, 정답 펼침과 본문 콘텐츠의 실제 높이 변화로 문제 박스가 늘거나 줄 때 140ms의 height 효과를 적용합니다. 부모 workspace의 가용 높이와 기존 최소/최대 높이를 유지하며, 한도에 도달하면 본문만 내부 스크롤합니다. 최초 진입과 답안 선택에 별도 높이 효과를 재생하지 않고, 문항 표시와 입력도 기다리지 않습니다. 빠른 변경은 이전 효과를 취소하고 현재 표시 높이에서 새 높이로 이어집니다. 768px 미만은 전체 화면 문제 박스를 유지하므로 높이 효과를 적용하지 않습니다. 브라우저 주소창, 화면 회전, 분할 화면과 키보드로 실제 viewport가 달라지면 높이 효과를 취소하고 즉시 크기를 맞춥니다. 동작 줄이기와 애니메이션 API 미지원 환경에서도 즉시 표시하며, 컴포넌트 종료 시 효과와 관찰자를 정리합니다.
...
페이지 이동과 클릭은 애니메이션 완료를 기다리지 않습니다. 페이지 퇴장 애니메이션, JS 타이머, 클릭 잠금이나 필터를 추가하지 않습니다. 검색 조건, 앵커, 페이지 내부 상태 변경으로 화면을 다시 마운트하지 않습니다. 실제 풀이 화면인 `app-focus-page`는 페이지 진입 효과에서 CSS로도 제외하고, 그 안의 문항 변경, 문제 박스 높이와 모바일 OMR만 위의 별도 규칙을 따릅니다. 랜딩에는 내부 페이지 효과를 중복 적용하지 않습니다.
...
`prefers-reduced-motion`에서는 페이지 진입, 탭 전환, 문항 변경, 문제 박스 높이, OMR 열기/닫기, 로딩과 랜딩 진입 애니메이션을 멈추고 hover의 이동도 없앱니다. 중앙 정렬 레이아웃이 탭 전환마다 흔들리지 않도록 `html`의 `scrollbar-gutter: stable`을 유지합니다. 문서 스크롤이 없는 활성 풀이의 `html.cbt-viewport`에서만 `auto`를 사용합니다.
