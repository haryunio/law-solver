...
`ProblemSetCardMetadata`는 전체 세션 수 옆에 `(2 풀이 중)`처럼 완료되지 않은 세션 수를 회색으로 표시합니다. 오프라인은 `in-progress`, 온라인은 `in_progress`와 `paused`를 포함합니다. 온라인은 세션이 있는 문제의 기존 세션 요약 API로 전체 수와 진행 중 수를 함께 집계하며 답안이나 문제 본문은 조회하지 않습니다. 문제 카드, 스켈레톤, 갤러리는 GNB의 한 줄 기준인 1024px 이상에서 2열, 그 아래에서 1열을 사용합니다. 과목 책장의 열 수는 별도 기준을 유지합니다.
