...
- 문제 카드는 `app-problem-card`의 순백색 표면과 중립 회색 `app-result-link` 이동 영역을 사용합니다. 문제 아래 풀이 기록은 온라인과 오프라인 모두 `SessionListItem`의 얇은 목록과 우측 CTA를 사용하며, 문제 카드에 미색 CTA 표면을 사용하지 않습니다.
