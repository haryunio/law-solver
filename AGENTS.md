...
- 과목 목록과 문제 대시보드의 상단 GNB는 `src/components/ui/DashboardHeaderTitle.tsx`를 사용합니다. 모바일에서도 브랜드명은 `Law Solver` 전체를 표시하고, 제목의 계층 구분에는 `|` 문자가 아니라 컴포넌트의 얇은 시각 구분선을 사용합니다. 모바일에서만 액션을 2열 전체 너비로 표시하고, `sm` 이상에서는 텍스트 너비의 버튼을 우측 정렬합니다. GNB는 `md`(768px) 이상에서 제목과 액션을 한 줄에 표시하며 버튼 너비를 유지하고 긴 제목에 말줄임표를 적용합니다. `md` 미만에서 제목 축소와 액션의 하단 배치를 함께 적용합니다. 두 줄인 `sm` 구간에서는 액션 영역의 가로 구분선 위아래에 카드 패딩과 같은 여백을 적용하며, `md`부터 구분선과 추가 여백을 제거합니다.
...
- 오프라인 문제 카드 정렬은 `useSettingsStore`의 `problemSortKey`(`title`/`created_at`)와 `problemSortDirection`(`asc`/`desc`)를 사용합니다. 기본은 등록 내림차순이며 기존 `law-solver-settings`에 저장합니다. 구형 설정이나 잘못된 정렬 값은 기본값으로 보완하고 `sortProblemSets`로 화면용 복사본만 정렬합니다. 과목 순서, 세션 순서와 실제 문항 풀이 순서는 변경하지 않습니다.
...
- 해설과 출처는 `LegalReferenceContent`로 표시합니다. `useSettingsStore.precedentLinkProvider`의 `off`/`law-go-kr`/`casenote`를 기존 설정 저장소에 보존하며 기본값과 구형/잘못된 값은 `law-go-kr`입니다. 출처는 `plainText`를 지정해 HTML을 해석하지 않습니다. 실제 링크 처리는 `RichTextContent`의 명시적 opt-in과 `src/lib/precedentLinks.ts`를 사용합니다. 2자리 또는 4자리 연도, 허용 사건부호, 일련번호가 한 텍스트 노드 안에 있는 경우만 연결하고 표시 문자열과 HTML 서식을 보존합니다. 국가법령정보센터 판례의 `evtNo`와 헌재결정례 검색의 `query`, 케이스노트 검색의 `q`에는 공백을 제거하고 URL 인코딩한 사건번호만 넣습니다. 새 탭 링크는 `noopener noreferrer`와 `no-referrer`를 유지하고 원본 HTML의 링크 주소나 이벤트 속성을 신뢰하지 않습니다. 문제/선지/보기에는 자동 링크를 붙이지 않으며 학습 원본, 백업, API DTO를 바꾸거나 자동 조회와 분석 이벤트를 추가하지 마세요.
...
풀이와 오답/전체/책갈피 복습은 `StandardUiScope`에 포함하지 않고 별도의 `app-study-page` 범위에서 같은 16px 카드, 12px 내부 박스/입력, 8px 태그 토큰을 명시적으로 사용합니다. 문제/OMR 패널은 `app-study-panel`, 보기와 보조 영역은 `app-study-inset`, 선지는 `app-study-option`, 태그와 입력은 `app-study-tag`와 `app-study-control`을 사용하세요. 미선택 선지에는 `app-study-option-neutral`을 추가하고 정답 emerald, 오답/선택 red, 정답 안내 blue, 책갈피 amber는 유지합니다. 이 계층은 표면과 테두리만 정리하며 기존 풀이 viewport, 내부 스크롤, 모바일 전체 화면, CTA 위치와 허용된 짧은 모션은 변경하지 않습니다.
...
- 문제 카드는 `app-problem-card`의 순백색 표면과 중립 회색 `app-result-link` 이동 영역을 사용합니다. 문제 유형, 문항 수와 세션 수는 `ProblemSetCardMetadata`의 동일한 너비(1:1:1)인 44px 정보 박스 한 줄로 표시합니다. 세션 수 옆에는 `(2 풀이 중)`처럼 완료되지 않은 세션 수를 회색으로 함께 표시하며 온라인의 일시 중단도 포함합니다. 문제 카드와 로딩 화면은 `lg`(1024px) 이상에서만 2열, 그 아래는 1열을 사용합니다. 카드 열 수는 GNB 전환 기준과 별도로 유지합니다. 오프라인 등록 시각은 하단 이동 영역의 `TimestampTag`에 넣습니다. 문제 아래 풀이 기록은 온라인과 오프라인 모두 `SessionListItem`을 사용합니다. 데스크톱 회차는 56px 정사각형으로 표시하고 통계와 CTA도 높이 56px로 맞춥니다. 위아래 여백은 16px, 제목과 태그 간격은 8px입니다. 회차, 태그 줄, 통계, CTA는 하단을 맞추고 태그가 줄바꿈되어도 유지합니다. 관리 메뉴 버튼은 정원형으로 카드에서는 32px, 목록에서는 데스크톱 28px와 모바일 32px를 사용하고 세로 중앙에 정렬합니다. CTA 폭은 모바일 96px, sm 이상 112px로 통일합니다. 모바일에서도 회차 왼쪽, 제목과 태그 오른쪽의 두 열을 유지합니다. 태그를 회차 아래 전체 폭으로 분리하지 않으며 640px 미만의 세션 날짜 태그는 달력 아이콘과 월일(`MM.DD`)만 표시하고 라벨, 연도와 시간은 생략합니다. 전체 날짜는 접근성 이름과 툴팁에 보존합니다. 이름 변경과 삭제는 `ActionMenu`에 모으고 하단 버튼 줄을 추가하지 않습니다. 문제 카드에 미색 CTA 표면을 사용하지 않습니다.
...
- 공통 `LandingHeader`는 최상단 72px 전체 폭 바에서 스크롤 48px 초과 시 64px 플로팅 카드로 바뀌고, 16px 이내로 돌아오면 복원됩니다. 16px 공통 곡률과 표면 토큰을 사용하고 모바일 안전 영역, 헤더 자리 보존, 앵커 상단 여백과 동작 줄이기를 유지하세요.
...
- 클라우드 백업/내려받기 창은 공통 `Dialog`의 body 포털로 렌더링합니다. 콘텐츠의 등장 효과나 탭 전환 영역 안에 fixed 레이어를 직접 넣지 않습니다. 작업 중 닫기 잠금과 종료 시 비밀번호/복구 캐시 초기화는 유지합니다.
...
- 활성 문제 풀이 화면은 `app-focus-page`를 사용합니다. 그라디언트, hover 이동·축소, smooth scroll과 일반적인 위치/크기 animation은 추가하지 않습니다. 기본 CTA는 단색 red-600, hover는 red-700을 사용하며 색상·테두리 전환은 90ms로 짧게 유지합니다. 예외는 `useQuestionPanelHeightMotion`의 내용에 따른 박스 높이 변경(768px 이상에서만 140ms), 공유 `QuestionContentMotion`의 문항 변경(120ms, opacity 0.88→1, 다음 문항은 오른쪽 4px에서 제자리, 이전 문항은 왼쪽 4px에서 제자리)과 `MobileOmrSheet`의 열기(160ms, 아래 12px에서 제자리)·닫기(120ms, 아래 8px로 이동하며 사라짐)뿐입니다. OMR에서 여러 문항을 건너뛸 때도 `questionIndex`로 이동 방향을 판단합니다. 문항 본문의 등장 효과는 문제 최초 진입, 답안 선택, 타이머, 정답 펼침에는 재생하지 않습니다. GNB, 문항 도구와 이전/다음 버튼에는 가로 이동 효과를 적용하지 않으며, 이전/다음 버튼은 박스 높이가 변할 때 하단을 따라갑니다. 자식 입력을 다시 마운트하거나 이동, 답안 저장, 스크롤을 지연하지 마세요. OMR은 닫기 시작 즉시 `pointer-events`와 `inert`로 조작을 해제하고, 빠른 전환은 이전 효과를 취소합니다. 동작 줄이기나 애니메이션 API 미지원 환경에서는 즉시 표시하며 실행 중 동작 줄이기를 켜도 효과를 취소합니다. 박스 높이는 문항 변경, 정답 펼침과 실제 콘텐츠 높이 변화에만 애니메이션을 적용하고 workspace 가용 높이와 기존 최소/최대 높이를 지킵니다. 768px 미만의 전체 화면 문제 박스에는 높이 효과를 적용하지 않으며 주소창, 회전, 화면 크기와 키보드로 viewport가 바뀌면 실행 중 높이 효과를 취소하고 즉시 맞춥니다. 이 예외를 복기 화면이나 페이지 진입 효과로 확대하지 마세요.
- 환경설정과 계정 진입 효과는 240ms/6px로 콘텐츠에만 적용합니다. 내부 탭은 `TabContentMotion`으로 이동 방향에 따라 160ms/8px의 가로 효과를 사용합니다. 최초 진입과 탭 전환 효과는 겹치지 않으며 GNB, 푸터, 모달은 움직이지 않습니다. 탭 효과를 위해 폼을 다시 마운트하거나 화면 표시를 지연하지 마세요. 빠른 전환은 이전 효과를 취소하고 동작 줄이기 설정과 애니메이션 API 미지원 환경에서는 즉시 표시합니다.
- 내부 페이지 진입 효과는 `src/app/RouteEntrance.tsx`의 명시적 허용 경로에서 목록의 `app-content-stagger` 직계 항목과 단일 영역의 `app-content-enter`에만 적용합니다. 메인 360ms/12px, 과목 목록 320ms/10px, 문제 목록 280ms/8px, 세션 목록 240ms/6px, 결과 조회 280ms/8px로 아래에서 올라오며 나타납니다. 항목별 시간차는 20ms씩 최대 100ms로 제한합니다. GNB는 즉시 바뀌고 배경, 푸터와 모달도 효과에서 제외합니다. GNB를 포함한 상위 컨테이너에 효과를 붙이지 마세요. 랜딩의 기존 700ms/22px 등장 및 미리보기 120ms 지연은 유지합니다. 실제 풀이와 모든 복기 경로는 페이지 진입 효과에서 제외하며, 활성 풀이 안의 문항 변경, 문제 박스 높이와 모바일 OMR 효과는 별도의 위 예외만 따릅니다. `prefers-reduced-motion`에서는 효과를 끕니다. 애니메이션 완료를 기다리는 이동, JS 타이머, 클릭 잠금을 추가하지 마세요. 종료 후 transform을 남기거나 검색 조건과 앵커 변경으로 재생하지 않습니다.
- 문제 풀이와 복기 화면은 동일한 `app-focus-page` 상단 바 높이를 사용합니다. 768px 이상에서는 기존 카드 여백을 유지하고, 그 아래의 활성 풀이만 GNB 바로 아래부터 문제 표면으로 채웁니다. 문제풀이 상단 우측 버튼은 고정 폭이나 균등 분할을 적용하지 않고 텍스트 너비만큼 차지한 상태로 우측 정렬합니다.
...
- 활성 풀이의 `cbt-solve-page`는 모든 화면 너비에서 실제 보이는 높이에 고정되는 flex column입니다. GNB는 실제 높이를 유지하고 `cbt-workspace`는 `flex: 1`, `min-height: 0`, `grid-template-rows: minmax(0, 1fr)`로 남은 공간을 사용합니다. 태블릿과 PC의 바깥 하단 여백은 `max(12px, safe-area-inset-bottom)`이며 문제와 OMR은 이 여백을 제외한 가용 높이를 넘지 않아야 합니다. 문서 전체 대신 문제 본문과 OMR 목록을 각각 내부 스크롤합니다.
- 768px 미만에서는 GNB 아래 전체 공간을 `--app-surface-problem` 표면으로 채우고 문제 카드의 외부 여백, 테두리, 곡률과 그림자를 없앱니다. GNB와 문제 도구 영역은 유지하고 `cbt-question-content`만 내부 스크롤합니다. 이전과 다음 버튼은 같은 너비의 52px 높이 플로팅 영역에 두며 좌우는 `max(16px, safe-area)`, 아래는 `max(12px, safe-area-inset-bottom)`만큼 띄웁니다. 본문의 하단 padding과 scroll-padding은 버튼 높이, 하단 간격, 추가 16px의 합으로 맞춰 마지막 내용이 가려지지 않게 합니다. OMR은 문제 우상단 `?` 왼쪽의 32px 원형 버튼으로 열고 플로팅 이동 버튼보다 위에 표시합니다.
- `useSolveViewport`는 모든 화면 너비에서 `visualViewport.height`와 `offsetTop`을 CSS 변수로 직접 반영해 주소창, 화면 회전, 분할 화면과 단답형 키보드에 맞춰 풀이 화면을 조절합니다. 미지원 시 `innerHeight`와 0을 사용하고, CSS의 초기 높이는 `100dvh`와 `100vh` fallback을 둡니다. 핀치 확대 중에는 마지막 치수를 유지합니다. 활성 기간의 `html.cbt-viewport`는 html/body 문서 스크롤을 잠그고, 768px 미만에서만 배경을 문제 표면색으로 통일합니다. 종료 시 클래스, 변수, 이벤트를 정리하며 `Dialog`와 충돌하지 않도록 body 인라인 overflow를 직접 변경하지 마세요. 전역 viewport 메타데이터는 유지합니다.
- 온라인 풀이의 `PremiumSolveSkeleton`도 같은 viewport 구조와 hook을 사용하고 문제 카드와 OMR을 가용 높이 이하로 제한합니다. 복기용 로딩은 `fitViewport={false}`로 제외하며 결과, 오답 확인, 전체 확인과 책갈피 확인에는 이 구조를 확장하지 않습니다. 검증에는 긴 본문과 해설의 끝까지 스크롤, OMR 마지막 문항 이동, 단답형 키보드 열기와 닫기, 태블릿 가로와 세로 전환, 분할 화면, 다크 모드와 PC 전환을 포함합니다.
- `제출 및 종료` 시 미응답 문항이 있으면 남은 개수를 표시하는 확인 모달을 먼저 띄웁니다. 미응답이 없으면 바로 제출하고, 확인 모달에서 취소하면 풀이 상태를 유지합니다.
- 768px 이상 활성 풀이에서 문제 카드와 OMR 높이는 독립적이며 각각 `max-height: 100%`로 workspace 가용 높이 안에 둡니다. 문제 카드의 최소 높이는 OX 420px, 단답형 360px, 5지선다 560px과 가용 높이 중 작은 값인 `min(기준 높이, 100%)`를 사용합니다. 내용이 길면 가용 높이까지 늘어나고 그 뒤에는 본문만 스크롤합니다. 5지선다 OMR은 `height: auto`로 문항이 적으면 내용만큼만 표시하며, OX와 단답형 OMR은 `height: 100%`를 사용합니다. OMR 제목과 다운로드 버튼은 축소하지 않고 목록만 내부 스크롤합니다. 복기 화면의 OMR도 문항 수가 적을 때 그리드 높이에 맞춰 강제로 늘리지 않습니다.
