...
- 탭 전환처럼 문서 높이가 달라지는 화면에서도 중앙 정렬 UI가 흔들리지 않도록 최상위 `html`의 `scrollbar-gutter: stable`을 유지합니다. 문서 스크롤을 잠그는 모바일 활성 풀이에서는 `cbt-mobile-viewport` 범위에만 `auto`를 적용합니다.
...
- 768px 미만의 활성 풀이에는 `cbt-solve-page`를 사용합니다. GNB 아래 전체 공간을 `--app-surface-problem` 표면으로 채우고 문제 카드의 외부 여백, 테두리, 곡률과 그림자를 없앱니다. GNB와 문제 도구 영역은 유지하고 `cbt-question-content`만 내부 스크롤합니다. 이전과 다음 버튼은 같은 너비의 52px 높이 플로팅 영역에 두며 좌우는 `max(16px, safe-area)`, 아래는 `max(12px, safe-area-inset-bottom)`만큼 띄웁니다. 본문의 하단 padding과 scroll-padding은 버튼 높이, 하단 간격, 추가 16px의 합으로 맞춰 마지막 내용이 가려지지 않게 합니다. OMR은 문제 우상단 `?` 왼쪽의 32px 원형 버튼으로 열고 플로팅 이동 버튼보다 위에 표시합니다.
- `useMobileSolveViewport`는 모바일에서 `visualViewport.height`와 `offsetTop`을 CSS 변수로 직접 반영해 단답형 키보드가 열린 공간에 풀이 화면을 맞춥니다. 미지원 시 `innerHeight`와 0을 사용하고, CSS의 초기 높이는 `100dvh`와 `100vh` fallback을 둡니다. 핀치 확대 중에는 치수를 갱신하지 않으며 데스크톱 전환 시 변수를 제거합니다. 활성 기간의 `html.cbt-mobile-viewport`는 모바일 html/body 배경과 스크롤 잠금만 관리하고 종료 시 클래스, 변수, 이벤트를 정리합니다. `Dialog`와 충돌하지 않도록 body 인라인 overflow를 직접 변경하지 마세요. 전역 viewport 메타데이터를 변경해 다른 화면에 안전 영역 처리를 강제하지 않습니다.
- 온라인 풀이의 `PremiumSolveSkeleton`도 같은 모바일 전체 화면 구조와 viewport hook을 사용합니다. 복기용 로딩은 `mobileFullScreen={false}`로 제외하며 결과, 오답 확인, 전체 확인과 책갈피 확인에는 이 모바일 구조를 확장하지 않습니다. 모바일 검증에는 긴 본문과 해설의 끝까지 스크롤, OMR 이동, 단답형 키보드 열기와 닫기, 화면 회전, 다크 모드와 768px 이상 복원을 포함합니다.
