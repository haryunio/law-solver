...
- 활성 문제 풀이 화면은 `app-focus-page`를 사용합니다. 그라디언트, hover 이동·축소, smooth scroll과 일반적인 위치/크기 animation은 추가하지 않습니다. 기본 CTA는 단색 red-600, hover는 red-700을 사용하며 색상·테두리 전환은 90ms로 짧게 유지합니다. 예외는 공유 `QuestionContentMotion`의 문항 변경(120ms, opacity 0.88→1, 다음 문항은 오른쪽 4px에서 제자리, 이전 문항은 왼쪽 4px에서 제자리)과 `MobileOmrSheet`의 열기(160ms, 아래 12px에서 제자리)·닫기(120ms, 아래 8px로 이동하며 사라짐)뿐입니다. OMR에서 여러 문항을 건너뛸 때도 `questionIndex`로 이동 방향을 판단합니다. 문제 최초 진입, 답안 선택, 타이머, 정답 펼침에는 효과를 재생하지 않고 GNB, 문항 도구와 이전/다음 버튼도 움직이지 않습니다. 자식 입력을 다시 마운트하거나 이동, 답안 저장, 스크롤을 지연하지 마세요. OMR은 닫기 시작 즉시 `pointer-events`와 `inert`로 조작을 해제하고, 빠른 전환은 이전 효과를 취소합니다. 동작 줄이기나 애니메이션 API 미지원 환경에서는 즉시 표시하며 실행 중 동작 줄이기를 켜도 효과를 취소합니다. 이 예외를 복기 화면이나 페이지 진입 효과로 확대하지 마세요.
